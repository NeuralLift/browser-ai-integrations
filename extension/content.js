...
const siteExtractors = {
  ...
  'web.whatsapp.com': () => {
    ...
    const messageElements = document.querySelectorAll('[data-pre-plain-text], .message-in, .message-out, [class*="message"]');
    ...
    const chatArea = document.querySelector('[data-tab="8"]') || 
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.action === 'getContext') {
    try {
      ...
      console.log('[Content] Sending context, text length:', content.text.length);
...
console.log('[Content] Browser AI Assistant content script loaded on:', window.location.hostname);
